...
#[instrument(skip(app), name= "broadcast")]
pub async fn broadcast_txs(app: Arc<App>) -> eyre::Result<()> {
    loop {
        broadcast_txs_run(app.clone()).await?;
        tokio::time::sleep(Duration::from_secs(5)).await;
    }
}

#[instrument(skip(app), name= "broadcast_run")]
async fn broadcast_txs_run(app: Arc<App>) -> eyre::Result<()> {
