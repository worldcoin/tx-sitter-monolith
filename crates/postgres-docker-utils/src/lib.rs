...
pub async fn setup() -> eyre::Result<DockerContainerGuard> {
    let container_id =
...
fn run_cmd_to_output(cmd_str: &str) -> eyre::Result<String> {
...
fn run_cmd(cmd_str: &str) -> eyre::Result<()> {
    run_cmd_to_output(cmd_str)?;

    Ok(())
}

fn parse_exposed_port(s: &str) -> eyre::Result<SocketAddr> {
    let parts: Vec<_> = s
        .split_whitespace()
        .map(|s| s.trim())
        .filter(|s| !s.is_empty())
