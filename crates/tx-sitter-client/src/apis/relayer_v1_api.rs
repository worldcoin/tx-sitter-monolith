...
pub async fn call_rpc(configuration: &configuration::Configuration, params: CallRpcParams) -> Result<serde_json::Value, Error<CallRpcError>> {
    ...
    let local_var_uri_str = format!("{}/1/api/{api_token}/rpc", local_var_configuration.base_path, api_token=crate::apis::urlencode(api_token));
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CallRpcError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}

pub async fn create_transaction(configuration: &configuration::Configuration, params: CreateTransactionParams) -> Result<models::SendTxResponse, Error<CreateTransactionError>> {
    let local_var_configuration = configuration;
    ...
    let local_var_uri_str = format!("{}/1/api/{api_token}/tx", local_var_configuration.base_path, api_token=crate::apis::urlencode(api_token));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());
...
pub async fn get_transaction(configuration: &configuration::Configuration, params: GetTransactionParams) -> Result<models::GetTxResponse, Error<GetTransactionError>> {
    let local_var_configuration = configuration;
    ...
    let local_var_uri_str = format!("{}/1/api/{api_token}/tx/{tx_id}", local_var_configuration.base_path, api_token=crate::apis::urlencode(api_token), tx_id=crate::apis::urlencode(tx_id));
...
pub async fn get_transactions(configuration: &configuration::Configuration, params: GetTransactionsParams) -> Result<Vec<models::GetTxResponse>, Error<GetTransactionsError>> {
    let local_var_configuration = configuration;
    ...
    let local_var_uri_str = format!("{}/1/api/{api_token}/txs", local_var_configuration.base_path, api_token=crate::apis::urlencode(api_token));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if let Some(ref local_var_str) = unsent {
        local_var_req_builder = local_var_req_builder.query(&[("unsent", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_user_agent) = local_var_configuration.user_agent {
        local_var_req_builder = local_var_req_builder.header(reqwest::header::USER_AGENT, local_var_user_agent.clone());
    }
